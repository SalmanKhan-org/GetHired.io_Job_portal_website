    ...
    return (
        <div className='container 2xl:px-20 mx-auto  flex flex-col lg:flex-row max-lg:space-y-8 py-8'>
            ...
            <div className='w-full lg:w-1/4 bg-white px-4'>
                        ...
                        <div className='mb-4 text-gray-600'>
                            {searchFilter.title && (
                                <span className='inline-flex items-center gap-2.5 bg-blue-50 border border-blue-200 px-4 py-1.5 rounded'>
                                    {searchFilter.title}
                                    <img  onClick={()=>setSearchFilter(prev => ({...prev,title:""}))}  src={assets.cross_icon} alt="cross_icon" />
                                </span>
                            )}
                            {searchFilter.location && (<span className=' ml-2  inline-flex items-center gap-2.5 bg-red-50 border border-red-200 px-4 py-1.5 rounded'>
                                ...
                                <img onClick={()=>setSearchFilter(prev => ({...prev,location:""}))} src={assets.cross_icon} alt="cross_icon" />
                            </span>)}
                        </div>
                    </>
                )}
                ...
                {/* Category Filter  */}
                <div className={showFilter ? "":"max-lg:hidden px-2"}>
                ...
                {/* Location Filter  */}
                <div className={showFilter ? "" : "max-lg:hidden mx-4"}>
            ...
            <section className='w-full lg:w-3/4 text-gray-800 max-lg:px-4'>
                <h3 className='font-medium text-2xl py-2 ' id='job-list'>Latest Jobs</h3>
                <p className='mb-8 text-sm'>Get your desired job from top companies</p>
                <div className='grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4'>
                    {filteredJobs?.slice((currentPage-1)*6, currentPage*6).map((job, index) => <JobCard key={index + job?._id} job={ job} />)}
                </div>
                {/* Pagination  */}
